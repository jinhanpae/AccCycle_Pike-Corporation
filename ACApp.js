// ACApp.js
// Generic journal entry practice engine for Accounting Cycle apps.
//
// Assumes these globals exist per exercise:
// - exerciseConfig (ACConfig.js)
// - journalQuestions (ACQuestions.js)
// - chartOfAccounts (ACChartOfAccounts.js)
//
...
// ---------- TB button enabling helpers ----------

function hasAnyAdjustingEntries() {
  for (const [qId, lines] of userEntriesByQuestion.entries()) {
    const q = qById.get(qId);
    if (q && q.isAdjusting === true && lines && lines.length > 0) {
      return true;
    }
  }
  return false;
...
function hasAnyClosingEntries() {
  for (const [qId, lines] of userEntriesByQuestion.entries()) {
    const q = qById.get(qId);
    if (q && q.isClosing === true && lines && lines.length > 0) {
      return true;
    }
  }
  return false;
...
function updateTrialBalanceButtons() {
  const tbAdjBtn = $("tb-adjusted-btn");
  const tbPostBtn = $("tb-postclosing-btn");

  if (tbAdjBtn) {
    const canShowAdjusted = hasAnyAdjustingEntries();
    tbAdjBtn.disabled = !canShowAdjusted;
    tbAdjBtn.title = canShowAdjusted
      ? "View adjusted trial balance."
      : "Enter at least one adjusting entry (Section B) to enable.";
  }

  if (tbPostBtn) {
    const canShowPostClosing = hasAnyClosingEntries();
    tbPostBtn.disabled = !canShowPostClosing;
    tbPostBtn.title = canShowPostClosing
      ? "View post-closing trial balance."
      : "Enter at least one closing entry (Section C) to enable.";
  }
}
  ...
  if (saved && saved.length > 0) {
    saved.forEach(line => {
      ...
      drInput.oninput = function () {
        ...
        // Only allow strictly positive amounts
        if (isNaN(v) || v <= 0) {
          this.value = "";
        } else {
  ...
  accountIds.forEach(accountId => {
    ...
    const transactionRows = lines
      .map(line => {
        ...
        let balanceDisplay = "";
        let balanceClass = "";     // Optional CSS class for zero balance
        if (runningBalance > 0) {
          balanceDisplay = runningBalance.toLocaleString();
        } else if (runningBalance < 0) {
          balanceDisplay = "(" + Math.abs(runningBalance).toLocaleString() + ")";
        } else {
          // runningBalance === 0 → show zero, and mark it as a cleared balance
          balanceDisplay = "0";
...
<head>
  ...
  <style>
    ...
    .zero-balance {
      color: #9ca3af;  /* soft gray hint for cleared balance */
    }
  </style>
</head>
<body>
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (chartBtn) {
    chartBtn.addEventListener("click", () => {
      window.open("ACChart.html", "COAWindow", "width=500,height=500");
    });
  }
  ...
  if (typeof exerciseConfig !== "undefined") {
    ...
    if (loginButtonLabelEl) loginButtonLabelEl.textContent = exerciseConfig.loginButtonLabel || "Start";
  }
